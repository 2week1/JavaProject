package com.kh.ch03_controll;

import java.util.Scanner;

public class ControllRun {

	public static void main(String[] args) {
		/*
		 * * 프로그램의 진행은 순차적으로 진행됨(위에서 아래, 왼쪽에서 오른쪽)
		 * 	 이 흐름을 바꾸고자 할 때, 제어문을 통해 직접 제어가 가능
		 * 
		 * 	- 선택적으로 실행시키고자 할 때 ->
		...
		if (num > 0) {		//조건식의 결과가 true 인 경우 if 중괄호 안의 코드 실행
			System.out.println(num +" 의 값은 양수다.");
			
		}else {				//조건식의 결과가 false 인 경우 else 중괄호 안의 코드 실행
	...
	*/
	 ...
	 */
		/*
		System.out.print("구매 할 과일을 입력하세요 (수박/참외/복숭아) :");
		 ...
		 *
		 *		- 조건식 : "반복문이 수행될 조건"
		 *				=> 조건식의 결과가 true일 때, 코드를 실행
		 *				   조건식의 결과가 false 일 때, 반복문 종료
		 *				=> 보통 초기식에 선언된 변수를 가지고 조건식을 작성
		 *
		 *		- 증감식 : 반복문을 제어하는 변수의 값을 증가시키거나 감소시키는 부분
		 *				=> 보통 초기식에 선언된 변수를 가지고 증감연산자(++ --) 를 사용한다.
		 *
		 ...
		 * 			*switch 문 안의 break;는 switch 문 만을 빠져나감!
		 */
		
		//1~10사이의 홀수만 출력 ==> continue;
		...
		// 반복적으로 랜덤값(1~100)을 추출한 이후 출력, 단 . 3의 배수인 경우 반복문 종료
		while(true) {

...
public class LotteryController {
    ...
    public TreeSet<Lottery> sortedWinObject() {
    	...
    	// 정렬된 결과를 반환
    	//	*TreeSet 생성자에게 정렬 기준을 정의한 SortedLottery객체의 인스턴스 전달
    	TreeSet<Lottery> sortedWin = new TreeSet<>(new SortedLottery());
    	sortedWin.addAll(win);
    	return sortedWin;
    	// 이때, 미리 만들어진 win을 가지고 정렬하기 때문에 당첨 대상 확인을 꼭 먼저 해야함.
	...
	public boolean searchWinner(Lottery l) {
		...
		//찾은 결과인 boolean 값 반환
		return win.contains(l);		// set참조변수.contains(객체) : 해당 Set내의 객체가 포함되어 있는지 여부 확인
	}

...
public BookController() {
	...
	public ArrayList<Book> searchBook(String keyword) {
		...
		for(int i=0; i<list.size(); i++) {		//[1] 검색 대상인 list(필드) 길이만큼 반복문 수행
			Book b = list.get(i);				//[2] 리스트에서 인덱스(i)에 해당하는 Book객체 조회
			String title = b.getTitle();		//[3] 해당 Book객체의 데이터 중 책 제목 조회
			
			if(title.contains(keyword)) {		//[4] keyword가 책 제목에 포함되어 있는지 확인
				//책 제목에 keyword 가 포함되어 있는 경우 searchList에 해당 객체 추가
				searchList.add(b);
			}
		}
		return searchList;
	...
	public Book delectBook(String title, String author) {
		...
		for (int i = 0; i<list.size(); i++) {
			...
			if( b.getTitle().equals(title) && b.getAuthor().equals(author)) {
				//리스트에서 책 제목과 저자명이 모두 동일한 객체일 경우
				removeBook = list.remove(i);
				break;
			}
		}
		return removeBook;		//remove(index) 해당 인덱스 위치의 객체를 제거하고 해당 객체를 반환
	}
